...
{
  if (fChain == 0) return;
  TFile *f = new TFile("ntuples_DYmadgh_2017_10_24.root", "RECREATE");
  ...
  TH1F *hnGVtxs = new TH1F("hnGVtxs", "no. of good primary vertices", 75, 0,75);
  ...
  puCalcGJ.Init("PU_histo_13TeV_GoldenJSON_69200nb.root");
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    double puweigj_ = (float) puCalcGJ.GetWeight(run, (*puTrue)[1]);
    PUwei = puweigj_;

    //cout << "puweigj_" << puweigj_ << endl;
    nvtx->Fill(nVtxs);
